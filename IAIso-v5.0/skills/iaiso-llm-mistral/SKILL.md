...
# Mistral middleware for IAIso

## When this applies

The agent calls Mistral directly (no orchestrator in between)
...
4. **Honour the provider's call boundary.** Mistral's tool-use API uses `parallel_tool_calls`; map each entry to a single IAIso `tool_call`. The provider can return multiple tool calls per response.
...
- `core/iaiso-python/iaiso/middleware/mistral.py`
- `core/iaiso-node/src/middleware/mistral.ts`
