---
name: iaiso-integ-llamaindex
description: "Use this skill when wrapping a LlamaIndex agent or pipeline with IAIso pressure governance. Triggers on `LlamaIndex`, `IAIsoQueryEngineWrapper`, or `iaiso.integrations.llamaindex`. Do not use for raw LLM-provider clients (see iaiso-llm-* skills) or for non-LlamaIndex orchestrators."
version: 1.0.0
tier: P2
...
# LlamaIndex integration for IAIso

## When this applies

Your team is running agents on LlamaIndex and you need every step
...
1. **Install the integration package.** The reference SDK ships
   ...
   ```python
   from iaiso.integrations.llamaindex import IAIsoQueryEngineWrapper
   ```

2. **Wrap at the orchestrator's outermost call boundary** — the
   one place every step of every agent passes through. For
   LlamaIndex that is documented in
   `vision/integrations/integ-llamaindex/README.md`.

   ```python
   safe = IAIsoQueryEngineWrapper(
       your_orchestrator,
...
3. **Attach a consent scope** before invoking, per
   `iaiso-runtime-consent-scope-check`. Do not run a LlamaIndex
...
Wrap the QueryEngine, not the LLM. LlamaIndex retrievers can fan out tens of subqueries per call; aggregate at the QueryEngine boundary.
