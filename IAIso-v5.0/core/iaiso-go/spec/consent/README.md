...
IAIso tokens are distinct from OIDC access tokens. The `iaiso.identity`
module in the Python reference shows how to map OIDC claims (`scp`,
`groups`, `roles`) into IAIso scopes and, if desired, mint an
IAIso-signed token from a verified OIDC identity. A normative
`spec/identity/` entry is planned for a subsequent MINOR spec release.
