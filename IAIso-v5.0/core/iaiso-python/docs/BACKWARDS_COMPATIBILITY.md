...
## Scope of the compatibility contract

The following areas sit outside the API-stability guarantee above:

- **Python version support.** The SDK tracks currently-supported
  upstream Python versions. A Python release reaching end-of-life
  does not constitute a breaking change from the SDK's side.
- **Transitive dependency APIs.** If `prometheus_client` changes its
  `Counter` class, the SDK's `PrometheusMetricsSink` adapts to match
  in a patch-level release.
- **Middleware compatibility across SDK major versions.** When a
  provider (e.g., Anthropic) releases a new SDK major version with
  breaking changes, the IAIso middleware ships an adapter for the new
  version — often as a new class (e.g., `AnthropicV5BoundedClient`)
  alongside the existing one so deployments can migrate on their own
  schedule.
