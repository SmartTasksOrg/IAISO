...
That's the whole pattern — 15 lines per call site. The SDK intentionally
leaves this layer open: a generic wrapper would either be a trivial
`requests.post` helper (adding nothing the user couldn't write) or
would try to abstract over every possible server shape (at which point
the wrapper's behavior drifts from the operator's actual deployment).
Keeping the integration local to the caller preserves clarity about
what is being counted and when.
